...
public class ImageOperations {
	...
	public  static boolean 					   belongsToSet(ArrayList<Pixel> connectComp, Pixel pixel) {
		Pixel curPixel;
		
		for(int i=0; i<connectComp.size(); i++) {
			curPixel = connectComp.get(i);
			if(curPixel.getX() == pixel.getX() && curPixel.getY() == pixel.getY())
				return true;
		}
	...
	private static ArrayList<Pixel> 		   contourPixels(Matrix imMatrix) {
		ArrayList<Pixel> contourPixels = new ArrayList<Pixel>();
		int w = imMatrix.getWidth(); int h = imMatrix.getHeight();
		Pixel curPixel, neigPixel;
		Vector neighbors;
		
		for(int y=0; y<h; y++) {
			for(int x=0; x<w; x++) {
				curPixel = (Pixel) imMatrix.get(x, y);
				if(curPixel.getHeight()!=0) {                   // foreground pixel 
					neighbors = curPixel.getNeighbours();
					for(int k=0; k<neighbors.size(); k++) {
						neigPixel = (Pixel) neighbors.get(k);
						if(neigPixel.getHeight()==0) {   	    // background neighbor
							contourPixels.add(curPixel);
							break;
						}
					}
				}
...
//					outputIm.putPixel(neighbour.getX(), neighbour.getY(), 255);
			        		...
			        		for(int x1=(xx-ws_d); x1<=(xx+ws_d); x1++) {
			        			if( (y1>=0) && (x1>=0) && (y1<height) && (x1<width)) {
			        				if(image.getPixel(x1,y1)==0) {
			        					...
			        					pixelValue = (31-ws_d)*(256/30) & 0xff;
	...
	public  static ImageProcessor 			   distanceTransform2(ImageProcessor imageproc) {
		ImageProcessor image = removeBorder(imageproc);
		//int out,max_graylevel=0,min_graylevel=255,pixelValue;
		//byte ws_d;
		int height = image.getHeight();
		int width  = image.getWidth();
		ImageProcessor distanceImage_aux = new ByteProcessor(width, height);
		ImageProcessor distanceImage;
		new ImagePlus("DistanceAux",distanceImage_aux).show();
		Matrix imMatrix = imageMatrix(image);
		Pixel curPixel, neigPixel;
		int pixelValue, distance;
		Vector neighbors;
		
		ArrayList<Pixel> contourPixels = contourPixels(imMatrix);
		
		for(int y=0; y<height; y++) {
			for(int x=0; x<width; x++) {
				curPixel = (Pixel) imMatrix.get(x, y);
				pixelValue = curPixel.getHeight();
				if(pixelValue == 0)
					distanceImage_aux.putPixelValue(x, y, 0);
				else
					distanceImage_aux.putPixelValue(x, y, width*height);
			}
		}
		
		for(int k=0; k<contourPixels.size(); k++) {
			curPixel = contourPixels.get(k);
			distanceImage_aux.putPixelValue(curPixel.getX(), curPixel.getY(), 1);
		}
		
		distanceImage = copyImage(distanceImage_aux);
		new ImagePlus("Distance",distanceImage).show();
		
		while(!contourPixels.isEmpty()) {
			curPixel = contourPixels.remove(0);
			neighbors = curPixel.getNeighbours();
			for(int k=0; k<neighbors.size(); k++) {
				neigPixel = (Pixel) neighbors.get(k);
				if(neigPixel.getHeight()!=0) {
					distance = (int) (Math.pow(curPixel.getX()-neigPixel.getX(),2) + Math.pow(curPixel.getY()-neigPixel.getY(),2))
							+ distanceImage_aux.get(curPixel.getX(), curPixel.getY()); 
					if( distance < distanceImage_aux.get(neigPixel.getX(), neigPixel.getY())) {   // new distance smaller than older distance
						distanceImage_aux.putPixelValue(neigPixel.getX(), neigPixel.getY(), distance);
						distanceImage.putPixelValue(neigPixel.getX(), neigPixel.getY(), 255 - distance*10);
						contourPixels.add(neigPixel);
					}
				}
			}
		}
	...
	private static Matrix					   imageMatrix(ImageProcessor imageproc) {
		int w = imageproc.getWidth(), h = imageproc.getHeight();
		int offset, topOffset, bottomOffset;
		int pixelValue;
		Matrix imageMatrix = new Matrix(w,h,new Pixel());
		Pixel currentPixel;
		...
		for(int y=0; y<h; y++) {
			for(int x=0; x<w; x++) {
				...
				imageMatrix.set(x, y, new Pixel(x, y, (byte) pixelValue));
				//currentPixel = (Pixel) imageMatrix.get(x, y);
				//currentPixel.setHeight(pixelValue);
			}
		}
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				if(y > 0) {
					if(x > 0)
						currentPixel.addNeighbour((Pixel)imageMatrix.get(x-1,y-1));  // topLeft
					currentPixel.addNeighbour((Pixel)imageMatrix.get(x,y-1));        // top
					if (x + 1 < w)
						currentPixel.addNeighbour((Pixel)imageMatrix.get(x+1,y-1));  // topRight
				}
				
				if (x > 0) 
					currentPixel.addNeighbour((Pixel)imageMatrix.get(x-1,y));        // left
				if (x + 1 < w) 
					currentPixel.addNeighbour((Pixel)imageMatrix.get(x+1,y));        // right
				
				if (y + 1 < h) {
					if (x > 0) 
						currentPixel.addNeighbour((Pixel)imageMatrix.get(x-1,y+1));  // bottomLeft
					currentPixel.addNeighbour((Pixel)imageMatrix.get(x,y+1));        // bottom
					if (x + 1 < w) 
						currentPixel.addNeighbour((Pixel)imageMatrix.get(x+1,y+1));  // bottomRight
				}
	...
	public  static ImageProcessor			   OR(ImageProcessor image1, ImageProcessor image2) {
			int width1 = image1.getWidth();
			int height1 = image1.getHeight();
			int width2 = image2.getWidth();
			int height2 = image2.getHeight();
			ByteProcessor imagesOR = null;
			int pixel1, pixel2;
	
			try {
				if(width1==width2 && height1==height2) {
			
					imagesOR = new ByteProcessor(width1,height1);
					for(int y = 0; y < height1; y++) {
						for(int x = 0; x < width1; x++) {
							pixel1 = image1.getPixel(x, y);
							pixel2 = image2.getPixel(x, y);
