...
public class MorphologicalOp {
	...
	private static ImageProcessor closeHoles(ImageProcessor image) {
		int width = image.getWidth(); int height = image.getHeight();
		//ImageProcessor imageBin = makebinary(ImageOperations.removeBorder(image));
		ArrayList<Pixel> connectComp;
		ArrayList<ArrayList<Pixel>> connectedComps = ImageOperations.findConnectedComponents(image);
		
		Pixel curPixel, firstPixel, lastPixel;
		int maxX,maxY,minX,minY;
		new ImagePlus("Image Bin", image).show();
		
		Matrix imageBinPixels = ImageOperations.pixelsMatrix(image);
		...
		for(int n=0; n<connectedComps.size(); n++) {
			...
			//  Search Hole
			if(minX==0) {
				firstPixel = firstPixelColumn(connectComp, 0);
				lastPixel  = lastPixelColumn(connectComp, 0);
				
				//holeFound = false;
				curPixel = (Pixel) imageBinPixels.get(firstPixel.getX(), firstPixel.getY());
				while(/*!holeFound &&*/ curPixel.getY()<=lastPixel.getY()) {
					if(curPixel.getIntHeight() == 0) {
						//holeFound = true;
						image.putPixelValue(curPixel.getX(), curPixel.getY(), 255);
						//break;
					}
					curPixel = curPixel.getBottomNeighbour();
					if(curPixel==null)
						break;
				}
			} 
			if(maxX==width-1) {
				firstPixel = firstPixelColumn(connectComp, maxX);
				lastPixel  = lastPixelColumn(connectComp, maxX);
			...
			if(minY==0) {
				firstPixel = firstPixelLine(connectComp, 0);
				lastPixel  = lastPixelLine(connectComp, 0);
				
				//holeFound = false;
				curPixel = (Pixel) imageBinPixels.get(firstPixel.getX(), firstPixel.getY());
				while(/*!holeFound &&*/ curPixel.getX()<=lastPixel.getX()) {
					if(curPixel.getIntHeight() == 0) {
					//	holeFound = true;
						image.putPixelValue(curPixel.getX(), curPixel.getY(), 255);
						//break;
					}
					curPixel = curPixel.getRightNeighbour();
					if(curPixel==null)
						break;
				}
			} 
			if(maxY==height-1) {
				firstPixel = firstPixelLine(connectComp, maxY);
				lastPixel  = lastPixelLine(connectComp, maxY);
	...
	public  static ImageProcessor fillHoles_Morpho(ImageProcessor image) {
		...
		ImageProcessor imageBin = closeHoles(makebinary(im));
		...
		return complement(markerImGeodDilated);
	}
	...
	public  static ImageProcessor fillHolesAtBorder(ImageProcessor image) {
		...
		for(int n=0; n<connectedComps.size(); n++) {
			...
			if(minX==0) {
				...
				while(/*!holeFound &&*/ curPixel.getY()<=lastPixel.getY()) {
					if(curPixel.getIntHeight() == 0) {
						//holeFound = true;
						imageHoles.putPixelValue(curPixel.getX(), curPixel.getY(), 255);
						//break;
					}
					curPixel = curPixel.getBottomNeighbour();
					if(curPixel==null)
		...
		while(!stable) {
			...
			imageHoles.erode();
			imageFilled = ImageOperations.AND(imageHoles, imageCompl);
		...
		return ImageOperations.OR(imageFilled,imageBin);
	}
	
	private static Pixel firstPixelColumn(ArrayList<Pixel> connectComp, int c) {
		Pixel firstPixel = null, curPixel;
		int minY = connectComp.size()*100, y;
	...
	private static Pixel lastPixelColumn(ArrayList<Pixel> connectComp, int c) {
		Pixel lastPixel = null, curPixel;
		int maxY = 0, y;
	...
	private static Pixel firstPixelLine(ArrayList<Pixel> connectComp, int l) {
		Pixel firstPixel = null, curPixel;
		int minX = connectComp.size()*100, x;
	...
	private static Pixel lastPixelLine(ArrayList<Pixel> connectComp, int l) {
		Pixel lastPixel = null, curPixel;
		int maxX = 0, x;
